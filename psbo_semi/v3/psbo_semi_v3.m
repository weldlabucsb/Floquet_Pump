...
frequencyRamp = 1;  %yes or no
modulation = 1; %0 is static lattice, 1 is modulated lattice
dfdt = 115;
sizex = 41;      % number of points in position space
sizep = 41;      % number of points in momentum space
...
for ii=1:length(tvec)
    x_avg(ii) = sum(sum(xvec(:,:,ii).*phasespaceprob,1));
    p_avg(ii) = sum(sum(pvec(:,:,ii).*phasespaceprob,1));
    xspread_avg(ii) = sqrt(sum(sum(((xvec(:,:,ii)-x_avg(ii)).^2).*phasespaceprob,1)));
    psquared_avg(ii) = sum(sum(((pvec(:,:,ii)-p_avg(ii)).^2).*phasespaceprob,1));
end
